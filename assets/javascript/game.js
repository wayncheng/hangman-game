$(document).ready(function() {
	...
	function init() {
		...
		if (race === 1) {
			...
			$("#lane4 .ln-decor").attr(
				"src",
				"assets/images/waves-ds-pink-200.svg"
			);
		} else if (race === 3) {
			keyArray = key3;
			$("#lane4 .ln-decor").attr(
				"src",
				"assets/images/waves-ds-yellow-200.svg"
			);
		} else if (race === 4) {
			keyArray = key4;
			$("#lane4 .ln-decor").attr(
				"src",
				"assets/images/waves-ds-red-200.svg"
			);
		} else {
			keyArray = key4;
			triplecrown = true;
			$("#lane4 .ln-decor").attr(
				"src",
				"assets/images/waves-ds-purple-200.svg"
			);
		}
		...
		for (var i = 0; i < key.length; i++) {
			var letNum = i;
			var letSpaceEl =
				'<span class="ltr-space" id="let' + letNum + '"></span>';
		...
		for (var i = 0; i < 6; i++) {
			wrongLetterHTML =
				'<span id="wrong' +
				i +
				'" class="ltr-space wrong-ltr inactive"></span>';
		...
		if (triplecrown === true) {
			$("#race-name").html(
				"<span>Congrats!</span> You won the Triple Crown!"
			);
		}
	} ////////////////////////// end init ////////////////////////////////////////////////
	...
	$(document).on("keyup", function play(event) {
		...
		$("#user")
			.attr("pos", userPos)
			.animate({ left: userPos + "%" }, 300);

		// Alpha CPU movement
		var cpuPos =
			startPoint + (totalStrikesAllowed - guessesLeft) * cpuIncrement;
		$("#cpu1")
			.attr("pos", cpuPos)
			.animate({ left: cpuPos + "%" }, 300);
